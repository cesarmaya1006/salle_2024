$(document).ready(function () {
    ...
    $(".form_calificar_fase_dos").submit(function (event) {
        ...
        if (valorCalificacion == 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $("#caja_loading_" + id_form).removeClass("d-none");
            ...
            Swal.fire({
                title:
                    "¿Ingresar la calificación con el siguiente valor : " +
                    valorCalificacion +
                    "?",
                text: "Esta acción no se puede deshacer!",
                icon: "warning",
    ...
    function submitRequest(form) {
        ...
        $.ajax({
            ...
            success: function (respuesta) {
                ...
                $("#caja_loading_" + respuesta.id).addClass("d-none");
                $("#span_calificacion_" + respuesta.id).html(respuesta.nota);
                $("#observacion_span_" + respuesta.id).html(
                    respuesta.observacion
                );
                $("#caja_componente_span_" + respuesta.id).removeClass(
                    "d-none"
                );
                Sistema.notificaciones(
                    "Componente calificado correctamente",
                    "Sistema",
                    "success"
                );
            },
